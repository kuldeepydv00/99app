...
const deleteUser = async (req, res) => {
  try {
    ...
    if (result.deletedCount === 1) {
      // Clean up in-memory caches if they exist
      if (typeof registeredUsers !== 'undefined') {
        const index = registeredUsers.findIndex(u => String(u._id || u.id) === String(id));
...
const getAppVersionConfig = async (req, res) => {
  ...
  res.json(appVersionConfig || {
    ...
    apkUrl: 'https://newmatkadomain.com/app-debug.apk',
...
const getPackages = async (req, res) => {
  res.json([
    { id: '1', packageName: 'com.example.numberbetting', appName: '99xmatka', version: '3.0', apkLink: 'https://newmatkadomain.com/app-debug.apk', status: 'Active' }
  ]);
};
